...
int st=0, t1=0, t2=0, mode=1,kenti=0,error=0,teisi=0,d_teisi=0,act=0;
...
/****************************************************************************/
		if(btn2==0 && btn1==1) {
			...
			while(1){
				...
				if(st==0 && d_power== 1 && dry_sec >= 10 ){													//脱水部　詰まり検知
					printf("詰まり検知\n");
					mot_state = MOT_Clean;
					//printf("%.3f sec\n", dry_sec);
					while(1){
				...
				if(st==0 && g_power== 1 && crash_sec >= 10 ){													//減容部　詰まり検知
					...
					while(1){
						...
						if(crash_sec >= 10){
							...
							printf("詰まり解消\n");
							if(st==0 && g_power== 1 && g_state==0)	mot_state = MOT_For;
							if(st==0 && g_power== 1 && g_state==1)	mot_state = MOT_Rev;
							break;
						}
					}
				}
